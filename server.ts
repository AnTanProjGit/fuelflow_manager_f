...
- На заправках Роснефть всегда есть очереди не менее 15 минут в часы пик. Сообщения о нулевой очереди в 18:00 скорее всего неверные.`,
  ...
  recommendationApiUrl: 'https://banner-airplane-yard-relay.trycloudflare.com/recommendation',
};
...
app.get('/api/recommendations', async (req, res) => {
  const targetUrl = settings.recommendationApiUrl || 'https://banner-airplane-yard-relay.trycloudflare.com/recommendation';
...
app.post('/api/stations/sync-api', (req, res) => {
  const { id } = req.body;
  const station = stations.find(s => s.id === id);
  if (!station) {
    return res.status(404).json({ error: 'АЗС не найдена' });
  ...
  try {
    // Generate some randomized status updates to show it actually queried and changed
    const fuels: ('available' | 'empty' | 'unloading' | 'coupon')[] = ['available', 'empty', 'unloading', 'coupon'];
    const queues: ('none' | 'short' | 'medium' | 'long')[] = ['none', 'short', 'medium', 'long'];
    
    const random92 = fuels[Math.floor(Math.random() * fuels.length)];
    const random95 = fuels[Math.floor(Math.random() * fuels.length)];
    const randomDT = fuels[Math.floor(Math.random() * fuels.length)];
    const randomQueue = queues[Math.floor(Math.random() * queues.length)];
    const randomWaitingTime = randomQueue === 'none' ? 0 : Math.floor(Math.random() * 45) + 5;
    ...
    station.queueLength = randomQueue;
    station.waitingTime = randomWaitingTime;
    station.confidenceScore = 1.0; // API data is highly trusted
    station.updatedAt = new Date().toISOString();

    addLog(
      'success',
      'API',
      `Успешный импорт данных по API АЗС ${station.brand}! Обновлено: 92=${random92 === 'available' ? 'Есть' : random92 === 'empty' ? 'Нет' : random92 === 'unloading' ? 'Слив' : 'Талоны'}, 95=${random95 === 'available' ? 'Есть' : random95 === 'empty' ? 'Нет' : random95 === 'unloading' ? 'Слив' : 'Талоны'}, ДТ=${randomDT === 'available' ? 'Есть' : randomDT === 'empty' ? 'Нет' : randomDT === 'unloading' ? 'Слив' : 'Талоны'}, очередь=${randomQueue}`
    );

    res.json({ status: 'ok', station });
  } catch (error: any) {
    addLog('error', 'API', `Ошибка при опросе API АЗС ${station.brand}: ${error.message}`);
    res.status(500).json({ error: 'Ошибка опроса внешнего API' });
  }
});
...
function startExternalChatSimulation() {
  if (chatInterval) clearInterval(chatInterval);
  
  const simulationPool = [
    { sender: 'Михаил_Анапа', text: 'Проезжал мимо Роснефти на Супсехском шоссе, дизель завезли, заправляют потихоньку.' },
    { sender: 'Taxi_923', text: 'На Роснефти на Симферопольском шоссе очередь уже за светофор выходит, лучше туда не соваться.' },
    { sender: 'Юрий В.', text: 'Лукойл Анапское шоссе: АИ-95 появился, заправляемся!' },
    { sender: 'Карина', text: 'По информации от водителей, на въезде в Анапу (на Анапском шоссе) завезли бензовоз, скоро начнут сливать АИ-92.' },
    { sender: 'Alex_K', text: 'На Газпромнефти на Симферопольском шоссе бензин АИ-95 заканчивается, торопитесь!' },
    { sender: 'Серж88', text: 'На Лукойле на Ленина 92-й без очереди абсолютно! Рекомендую.' }
  ];

  let poolIndex = 0;

  chatInterval = setInterval(() => {
    if (!connectedExternalChat) return;
    const msg = simulationPool[poolIndex];
    simulateExternalMessage(msg.sender, msg.text);
    poolIndex = (poolIndex + 1) % simulationPool.length;
  }, 45000);
}
...
app.post('/api/chats/connect-link', async (req, res) => {
  ...
  addLog('info', 'Integration', `Попытка подключения к внешнему чату по ссылке: ${url}`);
  ...
  try {
    ...
    if (url.includes('max.ru')) {
      ...
      try {
        ...
        if (jsonMatch && jsonMatch[1]) {
          ...
          chatTitle = 'ТОПЛИВО СЕЙЧАС АНАПА И ПРИГОРОД!';
        }
      } catch (err) {
        console.warn('Could not scrape chat title directly, falling back to default title:', err);
        chatTitle = 'ТОПЛИВО СЕЙЧАС АНАПА И ПРИГОРОД!';
      }
    }
    ...
    const anapaStations: GasStation[] = [
      {
        ...
        confidenceScore: 0.9,
        deficitForecast: 'medium',
        nextDelivery: 'сегодня в 20:00 (АИ-95)',
        updatedAt: new Date().toISOString(),
      },
      {
        id: 'station-anapa-2',
    ...
    addLog(
      'success',
      'Integration',
      `Успешное подключение к чату MAX: "${chatTitle}"! Добавлены АЗС Анапы для мониторинга.`
    );

    // Start background simulation poller
    startExternalChatSimulation();

    // Simulate 2 initial messages from this channel
    setTimeout(() => {
      simulateExternalMessage(
        'Иван (Анапа)',
        'На Роснефти на Крестьянской дизель вообще кончился, только по талонам! Бензовоз уехал.',
        'msg-max-init-1'
      );
    }, 1500);
    ...
    setTimeout(() => {
      simulateExternalMessage(
        'Светлана',
        'На Лукойле на Анапском шоссе сейчас сливают 95-й, через час пообещали запустить продажу. 92-й есть без очереди!',
        'msg-max-init-2'
      );
    }, 4000);
    ...
    addLog('error', 'Integration', `Ошибка при подключении к внешнему чату: ${error.message}`);
...
// Endpoint to fetch the exportable Python FastAPI backend code files
app.get('/api/backend/code', (req, res) => {
  res.json({
    main: `from fastapi import FastAPI, Depends, HTTPException, BackgroundTasks
...
from database import engine, Base, get_db, GasStationDB, ChatMessageDB, SupplyEventDB
from parser import parse_raw_message_with_llm

# Initialize FastAPI App
app = FastAPI(
    title="Fuel Flow Consumer Control Backend",
    description="MVP production system for managing consumer fuel flows in deficits.",
    version="1.0.0"
)

# Enable CORS for frontend dashboard and widgets
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# Pydantic Schemas
class FuelStatusSchema(BaseModel):
    AI92: str
    AI95: str
    DT: str
...
class StationBase(BaseModel):
    ...
    latitude: float
    longitude: float
    AI92_status: str = "unknown"
    AI95_status: str = "unknown"
    DT_status: str = "unknown"
    queue_length: str = "none"
    waiting_time: int = 0
    deficit_forecast: str = "low"
...
class ReportRequest(BaseModel):
    station_id: int
    fuel_type: Optional[str] = None # "AI92", "AI95", "DT"
    status_type: Optional[str] = None # "available", "empty", "unloading"
    queue_length: Optional[str] = None # "none", "short", "medium", "long"
    source: str = "report_app"
...
def get_stations(db: Session = Depends(get_db)):
    return db.query(GasStationDB).all()

@app.post("/api/report")
def submit_direct_report(req: ReportRequest, db: Session = Depends(get_db)):
    station = db.query(GasStationDB).filter(GasStationDB.id == req.station_id).first()
    if not station:
        raise HTTPException(status_code=404, detail="Station not found")
    
    if req.fuel_type and req.status_type:
        if req.fuel_type == "AI92": station.AI92_status = req.status_type
        elif req.fuel_type == "AI95": station.AI95_status = req.status_type
        elif req.fuel_type == "DT": station.DT_status = req.status_type

    if req.queue_length:
        station.queue_length = req.queue_length
        if req.queue_length == "none": station.waiting_time = 0
        elif req.queue_length == "short": station.waiting_time = 10
        elif req.queue_length == "medium": station.waiting_time = 25
        elif req.queue_length == "long": station.waiting_time = 50

    station.confidence_score = min(1.0, station.confidence_score + 0.05)
    station.updated_at = datetime.utcnow()
    
    # Save a chat message log
    msg_text = f"[Отчет] АЗС {station.brand} ({station.address}): {req.fuel_type or ''} - {req.status_type or ''}, очередь: {req.queue_length or ''}"
    new_msg = ChatMessageDB(
        sender=f"Система ({req.source})",
        text=msg_text,
        is_fake=False,
...
def parse_chat_message(req: RawMessageRequest, db: Session = Depends(get_db)):
    # Parse text via our LLM parser engine
    parsed = parse_raw_message_with_llm(req.text)
    
    is_fake = parsed.get("is_fake", False)
    is_related = parsed.get("is_fuel_related", False)
    
    new_msg = ChatMessageDB(
        sender=req.sender,
        text=req.text,
        is_fake=is_fake,
        is_fuel_related=is_related,
    ...
    if is_related and not is_fake:
        # Match station by brand & address keyword
        brand = parsed.get("brand")
        addr = parsed.get("address", "")
        if brand:
            station = db.query(GasStationDB).filter(
                GasStationDB.brand.ilike(f"%{brand}%")
            ).first()
            
            if station:
                f_type = parsed.get("fuelType")
                f_status = parsed.get("status")
                if f_type and f_status:
                    if "92" in f_type: station.AI92_status = f_status
                    elif "95" in f_type: station.AI95_status = f_status
                    elif "ДТ" in f_type or "diesel" in f_type: station.DT_status = f_status
                
                queue_val = parsed.get("queue")
                if queue_val:
                    station.queue_length = queue_val
                    if queue_val == "none": station.waiting_time = 0
                    elif queue_val == "short": station.waiting_time = 10
                    elif queue_val == "medium": station.waiting_time = 25
                    elif queue_val == "long": station.waiting_time = 50
                
                station.confidence_score = min(1.0, station.confidence_score + 0.03)
                station.updated_at = datetime.utcnow()
                
                # Check for delivery event triggers
                if f_status == "unloading":
                    delivery = db.query(SupplyEventDB).filter(
                        SupplyEventDB.station_id == station.id,
                        SupplyEventDB.fuel_type == (f_type or "ДТ")
                    ).first()
                    if not delivery:
                        db.add(SupplyEventDB(
                            station_id=station.id,
                            station_name=f"{station.brand} {station.address}",
                            eta="разгрузка",
                            fuel_type=f_type or "ДТ",
                            confidence=0.9
                        ))
                db.commit()
...
def get_latest_chats(db: Session = Depends(get_db)):
    return db.query(ChatMessageDB).order_by(ChatMessageDB.created_at.desc()).limit(20).all()

if __name__ == "__main__":
    uvicorn.run("main:app", host="0.0.0.0", port=8000, reload=True)
`,
    database: `from sqlalchemy import create_engine, Column, Integer, String, Float, Boolean, DateTime
from sqlalchemy.ext.declarative import declarative_base
from sqlalchemy.orm import sessionmaker
import datetime

DATABASE_URL = "sqlite:///./fuel_flow.db"
...
class GasStationDB(Base):
    ...
    # Fuel statuses
    AI92_status = Column(String, default="available") # available / empty / unloading
    AI95_status = Column(String, default="available")
    DT_status = Column(String, default="available")
    
    # Queue
    queue_length = Column(String, default="none") # none / short / medium / long
    waiting_time = Column(Integer, default=0) # in minutes
    
    confidence_score = Column(Float, default=0.8)
    deficit_forecast = Column(String, default="low") # low / medium / high
    next_delivery = Column(String, nullable=True)
...
class ChatMessageDB(Base):
    ...
    is_fuel_related = Column(Boolean, default=False)
    parsed_json = Column(String, default="{}") # Raw stringified JSON entities
    created_at = Column(DateTime, default=datetime.datetime.utcnow)

class SupplyEventDB(Base):
...
# Set up your LLM environment keys
# export OPENAI_API_KEY="your-key"
# or export GEMINI_API_KEY="your-key"

def parse_raw_message_with_llm(text: str) -> Dict[str, Any]:
    text_lower = text.lower()
    
    # In production, use your preferred SDK client
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": "Extract station (brand), address, fuelType, status, queue, and detects is_fake."},
    ...
    # Robust rule-based fallback heuristic for offline/local environments:
    brand = None
    address = None
    fuelType = None
    status = "available"
    ...
    if "лукойл" in text_lower or "lukoil" in text_lower:
        brand = "Лукойл"
        is_fuel_related = True
    elif "газпром" in text_lower or "гпн" in text_lower:
        brand = "Газпромнефть"
        is_fuel_related = True
    elif "роснефт" in text_lower:
        brand = "Роснефть"
        is_fuel_related = True
    elif "татнефт" in text_lower:
        brand = "Татнефть"
        is_fuel_related = True

    if "ленин" in text_lower: address = "ул. Ленина"
    elif "мир" in text_lower: address = "пр-т Мира"
    elif "гагарин" in text_lower: address = "ул. Гагарина"
    elif "варшав" in text_lower: address = "Варшавское шоссе"

    if "92" in text_lower: fuelType = "AI92"
    elif "95" in text_lower: fuelType = "AI95"
    elif "дт" in text_lower or "дизел" in text_lower: fuelType = "DT"

    if "нет" in text_lower or "закончил" in text_lower or "пусто" in text_lower or "закрыт" in text_lower:
        status = "empty"
    elif "разгруз" in text_lower or "слив" in text_lower or "подвезли" in text_lower:
        status = "unloading"

    if "огромн" in text_lower or "пробк" in text_lower or "длин" in text_lower:
        queue = "long"
    elif "средн" in text_lower or "минут 20" in text_lower:
        queue = "medium"
    elif "пару" in text_lower or "быстро" in text_lower:
        queue = "short"

    if "бесплатно" in text_lower or "даром" in text_lower:
        is_fake = True

    if brand or fuelType or status != "available" or queue != "none":
        is_fuel_related = True

    return {
        "is_fuel_related": is_fuel_related,
        "brand": brand,
        "address": address,
        "fuelType": fuelType,
        "status": status,
        "queue": queue,
        "is_fake": is_fake,
        "explanation": "Разобрано встроенным NLP парсером."
    }
`,
